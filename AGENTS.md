...
After every change to a script (`.gd`), scene (`.tscn`), `project.godot`, or `AGENTS.md`, conduct a code review — inspect the diff (`git diff`) against the conventions above (naming, member order, max line length) and verify the change is complete and consistent with the architecture.
...
| Path | Role |
|---|---|
| `scenes/main/main.gd` | Root scene manager — swaps the current screen (`_set_scene()`); preloads `menu.tscn`, `game.tscn`, `settings.tscn`, and `victory.tscn`; starts the game via `_start_game(player2_is_ai)` — remembers the mode in `_player2_is_ai` (used for victory restart), calls `set_player2_is_ai()` before adding the game to the tree, and connects the game's `game_over` to the victory screen and the menu's `settings_pressed` to the settings screen |
| `scenes/menu/menu.gd` | `MainMenu` (Control) — emits `multiplayer_pressed` / `vs_ai_pressed` / `settings_pressed` / `quit_pressed`; buttons `%MultiplayerButton`, `%VsAiButton` (side by side in a `ModeButtons` `HBoxContainer`), `%SettingsButton`, `%QuitButton` |
| `scenes/game/game.gd` | `GameScene` — root script of `game.tscn`; `@export ship_textures` (empty by default, filled in the inspector) — with at least 2 variants it picks 2 random distinct textures and assigns them to the players, otherwise warns without assigning; `set_player2_is_ai(is_ai)` (called before the scene enters the tree) switches `Player2` between human and `EnemyAI` control; forwards `game_over(winner_player_index, winner_score)` from `RespawnManager` and `restart_pressed` / `menu_pressed` from `PauseMenu` to `Main`. `Player2` in `game.tscn` has `is_ai = true` (the `Main` screen sets it explicitly per mode) |
...
| `scenes/player/player.gd` | `Player` ship (CharacterBody2D) — thrust, rotation, fire; hides on death and `respawn(spawn_position)` restores it; `set_ship_texture(texture)` swaps the ship sprite; keeps the `HealthBar` `ProgressBar` child upright and hovering above the ship by counter-rotating it each frame (`HEALTH_BAR_OFFSET`), updating its value from `Health.health_changed`; plays `ShootSFX` / `DamageSFX` / `ExplosionSFX` (three `AudioStreamPlayer2D` children of `player.tscn`, `bus` `SFX`) on fire, on damage (via `EventBus.ship_damage_received`), and on death; on damage also blinks the ship sprite red by tweening `flash_amount` of the `damage_flash` shader material, reading `blink_count` / `blink_duration` uniforms from it; public helpers `rotate_ship` / `set_thrusting` / `try_fire` are used both by `handle_input` and by the AI; `@export is_ai` (default `false`) swaps keyboard input for an `EnemyAI` child component added in `_ready` |
| `scenes/player/enemy_ai.gd` | `EnemyAI` (Node) — child of an AI-controlled `Player`; runs in `_process` (skips itself when the player is dead or the enemy is gone) and drives its parent via `rotate_ship` / `set_thrusting` / `try_fire`; re-plans its decisions every `think_interval` seconds, holding the previous course between ticks; targets the sibling `Player%d % (3 - player_index)` and keeps distance — approaches past `approach_range` on an angled course, retreats under `retreat_range - retreat_hysteresis`, and in between alternates engagement holds (face the enemy and fire) with perpendicular weaves (thrust across the line of fire, direction flipped each `strafe_interval`/`strafe_duration` phase cycle); approach and retreat carry a `strafe_blend` lateral component so the ship never flies in a straight line; fires with imperfect aim — lead scaled by `lead_accuracy`, cone `aim_tolerance` shrunk per shot by `aim_error`, shot rolled against `accuracy`, cadence jittered around `shoot_cooldown`, and no shot is attempted when the nose is beyond `aim_tolerance * COARSE_AIM_WINDOW`; dodges incoming enemy bullets (scans `Bullet` siblings, only threats arriving within `[reaction_time, dodge_horizon]` are dodged, escape perpendicular for `dodge_duration`) |
...
| `tests/test_ai_player.gd` | `EnemyAI` — enemy resolution, rotation toward / approach / retreat, weaving perpendicular to the enemy line in the combat band, firing only when aimed, dodging incoming bullets perpendicularly, standing down when the enemy is gone or the ship is dead; fairness behavior — decision latency (`think_interval`), no dodge for point-blank bullets (`reaction_time`), accuracy gating shots |
