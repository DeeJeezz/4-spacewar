...
| `scenes/player/player.gd` | `Player` ship (CharacterBody2D) — thrust, rotation, fire; hides on death and `respawn(spawn_position)` restores it; `set_ship_texture(texture)` swaps the ship sprite; plays `ShootSFX` / `DamageSFX` / `ExplosionSFX` (three `AudioStreamPlayer2D` children of `player.tscn`, `bus` `SFX`) on fire, on damage (via `EventBus.ship_damage_received`), and on death; on damage also blinks the ship sprite red by tweening `flash_amount` of the `damage_flash` shader material, reading `blink_count` / `blink_duration` uniforms from it |
| `scenes/player/damage_flash.gdshader` | Canvas-item shader on the player `Sprite2D` via a `ShaderMaterial` (`resource_local_to_scene = true` so the two players don't share it) — mixes the sprite toward `flash_color` by `flash_strength * flash_amount`; editor-tunable uniforms `flash_color` / `flash_strength` / `blink_count` / `blink_duration` / `flash_amount` |
